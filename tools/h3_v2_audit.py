            ...
            if isinstance(keywords.get("node_id"),ast.Constant) and keywords["node_id"].value==node_id:
                def inputs():
                    ...
                    for item in keywords["inputs"].elts:
                        ...
                        kind=item.func.value.attr;key=item.args[0].value;options={kw.arg:kw.value for kw in item.keywords}
                        default=None
...
def audit_workflow(path):
    ...
    media=[]
    for target in (7,14):
        ports=[row for row in nodes[target]["inputs"] if row.get("link") in links and links[row["link"]][1]==182]
    ...
    legacy=json.loads(nodes[172]["widgets_values"][0]);project=json.loads(nodes[165]["widgets_values"][0])
    name="_h3_v2_audit_source_" + uuid.uuid4().hex;package=types.ModuleType(name);package.__path__=[str(PLUGIN)];sys.modules[name]=package
    from importlib import import_module
    try:
        interview=import_module(name+".h3_focus.interview")
        compiled=interview.compile_interview(legacy,project);state=compiled["state"]
        preserved=all(state[key]==value for key,value in legacy.items() if key!="schema_version")
        assert preserved and interview.normalize_interview(state)==state
    finally:
        for key in tuple(sys.modules):
            if key == name or key.startswith(name + "."):
                sys.modules.pop(key, None)
    ...
    return {"sha256":original_hash,"node_count":len(nodes),"link_count":len(links),"structural_errors":errors,"contains_ZFI":any(row["type"].startswith("ZFI") for row in nodes.values()),"interview_to_outlet":endpoint(182,"reference_plan"),"fixed_outputs":len(nodes[182]["outputs"]),"LOW_HIGH_media":media,"canvas":{"desk_width":endpoint(165,"width"),"desk_height":endpoint(165,"height"),"LOW_width":endpoint(7,"width"),"LOW_height":endpoint(7,"height"),"HIGH_width":endpoint(14,"width"),"HIGH_height":endpoint(14,"height"),"HIGH_upscale_type":nodes[13]["type"],"preserved_HIGH_upscale":True},"length":{"LOW":endpoint(7,"length"),"HIGH":endpoint(14,"length"),"expression_type":nodes[30]["type"],"exact_expression_matches":nodes[30]["widgets_values"][0]==EXPRESSION,"a":endpoint(30,"values.a")},"migration":{"legacy_schema":legacy["schema_version"],"current_schema":state["schema_version"],"all_legacy_values_preserved":preserved,"idempotent":True,"private_text_exported":False,"first_detection_required":state["alignment"] is None and bool(compiled["call_references"])},"ownership":ownership,"unresolved_source_classes":unresolved,"model_selection_names_exported":False,"workflow_written":False}
...
def skeleton(mappings):
    ...
    interview=add(3,"ZVH3InterviewForm","Detect before generation",[("media_project","ZV_MEDIA_PROJECT")],cls_outputs("ZVH3InterviewForm"),defaults("ZVH3InterviewForm"))
    ...
    stage=add(9,"ZFPromptDirectorLocalLLM","Stage1 optional — connect your writer backend",stage_inputs,cls_outputs("ZFPromptDirectorLocalLLM"),defaults("ZFPromptDirectorLocalLLM"))
    ...
    wire(1,0,2,"width");wire(1,1,2,"height");wire(2,0,3,"media_project");wire(3,8,4,"reference_plan");wire(2,0,5,"media_project");wire(5,4,6,"values.a")
    for target in (7,8):
        wire(1,0,target,"width");wire(1,1,target,"height");wire(6,1,target,"length");wire(3,3,target,"prompt")
        for key,slot in media:wire(4,slot,target,key)
    wire(3,0,9,"role");wire(3,1,9,"prompt")
    for i in range(11):wire(4,i,9,f"image{i+1}")
    for i in range(3):wire(4,i+11,9,"video_frames"+(str(i+1) if i else ""))
    # Use the writer response for both conditioning prompts, preserving interview ancestry.
    for target in (7,8):
        old=next(row for row in links if row[3]==target and nodes[target-1]["inputs"][row[4]]["name"]=="prompt")
        nodes[2]["outputs"][3]["links"].remove(old[0]);old[1]=9;old[2]=0;stage["outputs"][0]["links"].append(old[0])
    graph={"last_node_id":9,"last_link_id":len(links),"nodes":nodes,"links":links,"groups":[],"config":{},"extra":{"generation_ready":False,"h3_example":"Preparation/wiring only. Stage1 response wiring is a preparation example; complete generation also requires the final H3 prompt processing chain, your writer backend, CLIP/VAEs/models and sampling/upscale chain. Adding models alone does not complete the three-stage flow. Not one-click generation."},"version":.4}
    for node in nodes:
    ...
    validate = mappings['ZVH3InterviewForm'].build.__globals__['validate_reference_hub_wiring']
    ...
    return graph,{"verified_classes":[row["type"] for row in nodes],"copied_plugin_classes":[key for key in mappings if key in {row["type"] for row in nodes}],"static_real_core_T8_schema":schemas,"media_wires_per_conditioning":22,"actual_plugin_input_output_and_T8_autogrow_checked":True,"backend_fixed_hub_wiring_errors":wiring['errors'],"no_test_stub_serialized":True,"generation_ready":False,"needs":"final H3 prompt processing chain, your writer backend, CLIP, video/audio VAEs, models and sampling/upscale chain"}
