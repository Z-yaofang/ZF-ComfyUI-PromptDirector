"""Build a separate long-video V1 example from a working H3 focus workflow."""
...
def _sanitize_desk(desk):
    ...
    for key in ("assets", "picture_track", "video_track", "audio_track"):
        project[key] = []
    project["outlet_slots"] = {"version": 1, "items": []}
...
def _rewrite_notes(nodes, *, mask_mode=False):
    ...
    content = [
        ...
        (
            ...
            "• 分段 LLM 只收到文字连续性说明，不会看到上一段实际尾帧或音频；实际 Guide 由执行节点和 AddGuide 接线处理。\n\n"
        ...
        (
            "③ 测试版边界｜先看再跑",
            "长视频功能仍是测试版；示例尚未在 RunningHub 验证。工作流已接好 LOW/HIGH AddGuide，但外部 T8 节点是否实际消费 Guide 必须以目标环境的运行结果为准。\n\n"
            + (
                "本 C1 支路把冻结源 IMAGE、实际 MASK 与原声接入 H3 nested AV latent；LOW→HIGH 边界恢复同一 1x 源 latent 与目标网格 MASK，解码后按像素回贴黑区源画面，Recorder 只接回贴后的画面与逻辑 final_audio。\n\n"
                if mask_mode else
                "下方蒙版支路只演示冻结源时钟、标准 MASK 绑定与逐段取片协议；它没有接入当前普通生成路径。需要实际 H3 蒙版 latent 时请使用独立 C1 蒙版工作流。\n\n"
            )
            + "ImageToMask 仍只是标准 MASK 接口示例，不能代替 SAM3、SeC 或手绘遮罩。SAM3+SeC 是外部依赖，未随插件或示例打包；当前 C1 只支持整次运行共享同一 raw MASK、同一来源/采样指纹和 LOW/HIGH 同画布 1x。",
        ),
    ]
...
def build(workflow, *, mask_mode=False):
    ...
    old_interview = _one(nodes, "ZVH3InterviewForm")
    ...
    old_saver = savers[0]
    debug_outputs = {node["id"] for node in nodes if node.get("type") == "easy showAnything"}
    ...
    new_nodes = [
        ...
        make_node(ids["entry"], "ZVLongVideoExecutionEntry", "每次只取当前段与前段 guide", [x + 2100, y + 850],
                  ...
                  [("segment_context", "ZV_SEGMENT_EXECUTION_CONTEXT"), ("reference_plan", "ZV_H3_REFERENCE_PLAN"),
                   ("system_prompt", "STRING"), ("stage1_task", "STRING"), ("stage2_prefix", "STRING"),
                   ("stage3_prefix", "STRING"), ("duration_seconds", "FLOAT"), ("frame_count", "INT"),
        ...
        make_node(ids["save"], "SaveVideo", "保存完整长视频", [x + 6360, y + 850],
                  ...
                  [("video", "VIDEO")], values=["MiniMaxH3/long_video_v1", "auto", "auto", "auto"],
                  named_values={"filename_prefix": "MiniMaxH3/long_video_v1", "format": "auto", "format.codec": "auto", "codec": "auto"},
    ...
    interview_map = {
        "system_prompt": "system_prompt", "stage1_task": "stage1_task", "stage2_prefix": "stage2_prefix",
        "stage3_prefix": "stage3_prefix", "duration_seconds": "duration_seconds", "reference_plan": "reference_plan",
    }
    ...
    extra["workflow_title"] = "H3焦点访谈 长视频蒙版 C1" if mask_mode else "H3焦点访谈 长视频分段 V1"
    extra["zv_long_video"] = {
        "version": 1,
        ...
        "llm_continuity": "text instruction only; prior frames/audio are applied by execution nodes and external AddGuide nodes",
        "mask_status": (
            "C1 full-frame source IMAGE/MASK/AUDIO is connected at LOW, restored after HIGH reconcile on the same 1x canvas, and pixel-composed after decode; SAM3/SeC are optional external dependencies and are not bundled"
            if mask_mode else
