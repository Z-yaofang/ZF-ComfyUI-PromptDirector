...
def normalize_execution_plan(value):
    ...
    for index, (segment_value, row_value) in enumerate(zip(segments, rows)):
        ...
        for field in ("user_prompt", "material_context_json"):
...
def segment_context(execution_plan, segment_index):
    ...
    material_context = json.loads(row["material_context_json"])
    material_context["long_video"] = {
        "segment_index": index,
        "segment_count": len(plan["segments"]),
        "segment_id": segment["segment_id"],
        "seam": segment["seam"],
        "global_start_frame": segment["start_frame"],
        "global_end_frame": segment["end_frame"],
        "incoming_guide": None if incoming_guide is None else {
            **incoming_guide,
            "local_start_seconds": 0.0,
            "local_end_seconds": incoming_guide["frame_count"] / plan["fps"],
            "global_start_seconds": incoming_guide["global_start_frame"] / plan["fps"],
            "global_end_seconds": incoming_guide["global_end_frame"] / plan["fps"],
        },
    }
    return {
        "schema_version": 1,
        ...
        "material_context_json": json.dumps(material_context, ensure_ascii=False, indent=2, allow_nan=False),
