...
def segment_project(plan, segment_id):
    ...
    included_ids = {row.get("item_id", row.get("clip_id")) for key in ("picture_track", "video_track", "audio_track") for row in project[key]}
    project["outlet_slots"]["items"] = [row for row in project["outlet_slots"]["items"] if row["binding_id"] is None or row["binding_id"] in included_ids]
