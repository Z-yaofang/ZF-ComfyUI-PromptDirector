...
![H3 长视频分段台的本地界面示例](images/h3-segment-desk-example.png)

截图仅展示分段台的操作区域；其中的素材名称和数值是单次本地示例，不是默认设置或生成验收结果。
...
更换素材后，先在分段台重新“获取素材台三轨”并排列，再把遮罩视频源的 `clip_id` 改为当前视频片段的稳定 ID，最后按当前素材整理采访内容并重新“检测并对齐全部分段”。旧工作流里手填的片段 ID 不会随素材自动变更；显示“所选视频片段不在冻结素材工程中”时，先核对这个 ID，而不是重装模型。旧采访对齐和旧素材用途也不能直接复用。
