...
H3 长参考重叠语义由本仓库的预设规则与长视频计划器共同实现：预设保存请求值和 `h3_guide` 策略，`long_video/plan.py` 派生合法重叠、步长、段数及短尾回摆。操作时应按稳定、可辨识的动作区选择重叠，并以分段台显示的实际 Guide 帧数为准；参见[长视频指南](LONG_VIDEO_GUIDE.md)。
