...
Visual proxy frames have no sound. If the first-stage evidence says audio was not heard, and the user did not supply its content, do not describe or rule out music, beats, speech, lyrics, ambience, or sound effects. A requested direct audio copy establishes only that the signal is copied unchanged and synchronized; it does not establish what it contains. In overall_soundscape, describe that copy relationship without guessing its contents. In non_diegetic_music, write that no new score is added and any music already present in the copied <Audio N> remains unchanged and uncharacterized. Do not write N/A for an uninspected copied soundtrack: that would imply music is absent. Do not turn an inferred visual dance rhythm into an observed audio beat.
...
Define any <Video N> used as the editing source and any <Audio N> used as a copied or referenced track on separate lines in subject_definitions; sound is never a <Subject N>. A costume-only or identity-only image belongs inside the applicable <Subject N> definition, not as a claimed first/last frame. In retention_analysis, give each relationship marker a short reason rather than leaving a bare marker.
...
Literal output syntax is mandatory: the six heading lines above must each end with a colon, including subject_definitions: and non_diegetic_music:. The summary task types inside square brackets must be joined with ' + ', never commas. The opening shot starts with [Shot 1] and no time marker; never write [Shot 1] At 00:00.000. Do not add markdown headings, a colon-free variant, or a seventh section. Check these punctuation rules immediately before returning the final answer.
"""
