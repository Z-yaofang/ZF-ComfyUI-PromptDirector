...
- `ZF 人像提示词生成器` is reusable across image models and assembles editable portrait prompts without reverse analysis. With `quantity = 1` it outputs the current prompt; larger values return a list of that size, keeping the current selection for item one and randomizing later items while preserving every lock, so downstream nodes can render consecutive images directly. The picker supports home pins, section randomize/lock/disable controls, per-field locks, in-place asset editing, and restoration of built-in descriptions.
