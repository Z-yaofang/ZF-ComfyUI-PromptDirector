...
def test_minimal_migration_preserves_crlf_widgets_properties_and_connections():
    raw = ('{\r\n  "nodes": [{"id":166,"type":"ZFUniversalMediaEvidenceDesk","pos":[1,2],"inputs":[],"outputs":[{"type":"STRING","links":[315]}],'
           '"properties":{"Node name for S&R":"ZFUniversalMediaEvidenceDesk","note":"ZFUniversalMediaEvidenceDesk"},'
           '"widgets_values":["ZFUniversalMediaEvidenceDesk",{"id":7,"type":"ZFUniversalMediaEvidenceDesk"}]}],\r\n'
           '  "links":[[315,166,1,42,0,"STRING"]],"other":{"type":"ZFUniversalMediaEvidenceDesk"}\r\n}\r\n').encode()
    migrated, changes, checks = M.migrate_bytes(raw)
    expected = raw.replace(b'"id":166,"type":"ZFUniversalMediaEvidenceDesk"', b'"id":166,"type":"ZVUniversalMediaEvidenceDesk"').replace(b'"Node name for S&R":"ZFUniversalMediaEvidenceDesk"', b'"Node name for S&R":"ZVUniversalMediaEvidenceDesk"')
...
def test_api_class_type_and_unicode_escaped_token():
    raw = b'{"4":{"class_type":"\\u005aFUniversalMediaEvidenceDesk","inputs":{"text":["3",0]},"_meta":{"title":"User title"}}}'
    result, changes, _ = M.migrate_bytes(raw)
    assert json.loads(result)["4"]["class_type"] == "ZVUniversalMediaEvidenceDesk"
...
def test_unrelated_properties_and_text_are_not_blindly_replaced():
    raw = b'{"nodes":[{"id":1,"type":"Other","properties":{"Node name for S&R":"ZFUniversalMediaEvidenceDesk"},"widgets_values":["ZFUniversalMediaEvidenceDesk"]}],"links":[]}'
    result, changes, _ = M.migrate_bytes(raw)
...
def test_dry_run_has_no_side_effects(tmp_path):
    source = tmp_path / "matched.json"; raw = b'{"nodes":[{"id":1,"type":"ZFUniversalMediaEvidenceDesk"}],"links":[]}'
    source.write_bytes(raw)
...
def test_retired_focus_compiler_is_not_migrated_or_revived():
    assert M.IDS == {"ZFUniversalMediaEvidenceDesk": "ZVUniversalMediaEvidenceDesk"}
    raw = b'{"nodes":[{"id":166,"type":"ZFH3FocusCompiler","properties":{"Node name for S&R":"ZFH3FocusCompiler"}}],"links":[]}'
    result, changes, checks = M.migrate_bytes(raw)
    assert result == raw and changes == [] and checks == {}
    raw = b'{"166":{"class_type":"ZVH3FocusCompiler","inputs":{}}}'
    assert M.migrate_bytes(raw) == (raw, [], {})
