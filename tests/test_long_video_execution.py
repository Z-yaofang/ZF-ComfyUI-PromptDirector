...
@pytest.mark.parametrize("field", ["user_prompt", "material_context_json"])
def test_h3_execution_rejects_non_24fps_and_tampered_compiled_prompt(field):
    plan = PLAN.build_segment_plan(CONTRACT.empty_project(), {
    ...
    valid["segments"][0][field] += " 已被篡改"
    with pytest.raises(EXECUTION.ExecutionPlanError, match="执行指纹"):
        EXECUTION.normalize_execution_plan(valid)


def test_execution_entry_exposes_only_current_prompt_contract(tmp_path, monkeypatch):
    monkeypatch.setattr(NODES, "_temp_root", lambda: str(tmp_path))
    entry = NODES.ZVLongVideoExecutionEntry()
    assert entry.RETURN_NAMES == (
        "segment_context", "reference_plan", "user_prompt", "material_context_json",
        "duration_seconds", "frame_count", "model_length", "model_adapter", "has_guide",
        "guide_frames", "guide_audio", "guide_frame_idx", "segment_id", "report",
    )
    plan = execution_plan(segment_count=1)
    output = dict(zip(entry.RETURN_NAMES, entry.enter(plan, 0), strict=True))
    assert len(entry.RETURN_NAMES) == len(entry.RETURN_TYPES) == 14
    assert output["user_prompt"] == plan["segments"][0]["user_prompt"]
    assert json.loads(output["material_context_json"])["long_video"]["incoming_guide"] is None
    assert output["has_guide"] is False
    assert output["guide_frames"] is None and output["guide_audio"] is None


def test_previous_segment_continuity_is_material_fact_not_form_instruction():
    plan = execution_plan(overlap_frames=22)
    context = EXECUTION.segment_context(plan, 1)
    material = json.loads(context["material_context_json"])
    continuity = material["long_video"]
    assert continuity["seam"] == "guide"
    assert continuity["segment_index"] == 1
    assert continuity["incoming_guide"]["source_segment_id"] == plan["segments"][0]["segment_id"]
    assert continuity["incoming_guide"]["local_end_seconds"] == 22 / 24
    assert context["user_prompt"] == plan["segments"][1]["user_prompt"]
    assert "连续性约束" not in context["user_prompt"]
    assert set(context).isdisjoint({"system_prompt", "stage1_task", "stage2_prefix", "stage3_prefix"})
    assert material["materials"] == []

    hard_cut = EXECUTION.segment_context(execution_plan(overlap_frames=0), 1)
    continuity = json.loads(hard_cut["material_context_json"])["long_video"]
    assert continuity["seam"] == "hard_cut"
    assert continuity["incoming_guide"] is None
...
def test_disk_backed_nodes_encode_probe_restore_guide_and_native_concat(tmp_path, monkeypatch):
    ...
    assert second[8] is True and second[9].shape == (22, 24, 32, 3)
    assert torch.allclose(second[9], torch.full_like(second[9], .2), atol=1 / 255)
