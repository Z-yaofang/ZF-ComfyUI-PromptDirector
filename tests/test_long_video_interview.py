...
def test_align_all_then_exclude_is_physical_and_invalidates_snapshot():
    ...
    assert "person.png" not in aligned["segments"][1]["material_context_json"]
...
def test_segment_form_only_emits_user_text_and_material_facts():
    ...
    for row in result["segments"]:
        assert "人物缓慢转身。" in row["user_prompt"]
        assert "保持蓝色外套" in row["user_prompt"]
        assert "未指定" not in row["user_prompt"]
        assert set(row).isdisjoint({"system_prompt", "stage1_task", "stage2_prefix", "stage3_prefix"})
        context = json.loads(row["material_context_json"])
        assert context["duration_seconds"] == row["duration_seconds"]
        assert context["target_frames"] == row["frame_count"]
        assert context["materials"] == [
            item for item in context["materials"] if item["item_id"] in {"p1", "v1"}
        ]
