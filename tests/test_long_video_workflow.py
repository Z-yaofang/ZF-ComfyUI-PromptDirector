...
EXAMPLE = ROOT / "docs" / "examples" / "H3焦点访谈_长视频分段_V1.json"
MASK_EXAMPLE = ROOT / "docs" / "examples" / "H3焦点访谈_长视频蒙版_C1.json"
...
def test_builder_rewrites_real_h3_graph_with_dynamic_ids_and_named_ports():
    ...
    assert not ({"ZVH3InterviewForm", "ZVProcessingWindowOutlet", "VHS_VideoCombine", "easy showAnything"}
    ...
    assert "hard_cut 不读取上一段尾部" in note_text and "只收到文字连续性说明" in note_text
    ...
    assert "text instruction only" in metadata["llm_continuity"]
