...
API② 与提示词整理器之间另有 `ZF 最终文本列表缓存（跨队列复用）`，提示词整理器再把结果送到最终展示区。它保存的是整组多任务成品提示词，不会像单文本缓存那样只留下最后一条；先“更新缓存”，再“使用缓存”会跳过 API②，直接把上一组结果送入后续整理和展示。旧的提示词总开关、字符串处理和字符串转列表链不再需要。
