...
面向 ComfyUI 的图像提示词、人像提示词、Music3、H3 和 Animate 工作流工具。素材编辑、需求收集、模型反推和视频执行各有独立边界，不需要把全部节点放进同一条工作流。

先看[节点用途与当前入口](docs/NODE_GUIDE.md)，按任务选择需要的部分。当前 H3 表格只有 **ZV H3 采访表 · 收集对齐整理**（`ZVH3InterviewFormV2`）；还输出 `system_prompt`、`stage1_task` 的旧表已废弃。

`custom_nodes` 中只放一份正式插件。Git worktree 和备份副本必须放在该目录外，否则多份 Python 节点和前端扩展可能同名注册、互相覆盖。清除重复安装后，先保存工作流，再重启 ComfyUI 并刷新浏览器。旧工作流中的废弃节点不会自动变成新端口，须按当前模板重新连接。
...
- 可选接入 11 路图片和三路视频抽帧批次，可完整覆盖 H3 的首帧、尾帧与 9 路参考图。图片保持比例缩放；每路视频独立抽样，只分析画面，不读取音轨。
- `force_offload` 关闭时可减少连续多任务之间的重复装载；随后要运行大型成图或视频模型时可开启，以释放显存。

### ZV 素材台与标准媒体出口

![本地素材台与 Animate 分段配对台界面示例](docs/images/media-animate-desk-example.png)

这是本地界面示例；截图中的演示素材、帧数和模型参数不是插件默认值，也不是云端运行验收结果。H3 的[长视频分段台](docs/LONG_VIDEO_GUIDE.md)和[采访表](docs/H3_INTERVIEW.md)另有界面截图。

当前仓库已包含 H3/素材台实现。素材台在原有两个工程输出之外，新增独立的 **原素材来源** 输出；素材池选中素材后点击 **发送原素材**，自动创建并真实连接原图片、原视频或原音频出口，读取完整原素材，不受时间轴裁剪、H3 对齐或采访表验证影响。原视频输出原生 `VIDEO`，也提供原文件路径供兼容的加载节点使用。详见 [原素材来源绑定](docs/H3_V2_07_SOURCE_BINDING.md) 和 [云端测试准备](docs/CLOUD_TESTING.md)。更新后正常重启 ComfyUI 并刷新前端；云端 GPU 验证尚未完成。

`ZV 通用素材取证台` 保留两个工程输出，并增加可选的 `width` / `height` 输入。两者连接实际生成画布后，视频会在解码阶段直接适配该尺寸，所有下游视频出口统一继承，不再逐个重复接宽高。选中已入轨素材后，检查器只显示一个明确动作，例如“发送图片6 / 发送视频1 / 发送音频1”；点击后创建绑定该素材稳定 ID 的小出口，同一出口可向任意数量的下游节点分线，重复点击会复用已有出口。发送只修改画布，不会排队生成或请求媒体解码。


- **ZV 图片素材出口**：完整原尺寸 `IMAGE`。
- **ZV 视频素材出口**：当前窗口交集的 CFR `IMAGE` 帧批次及配对原声 `AUDIO`；原声停用或不存在时该口为 `None`。
- **ZV 音频素材出口**：独立音频片段的窗口交集，标准 `AUDIO`。
- **ZV 时间线混音出口**：整个窗口的已启用音频混音，保留静音间隔与相对位置。

出口端口固定，数量随需要创建，只有参与执行的出口才解码。所有 AUDIO 统一为 44100 Hz 双声道，视频使用窗口参考 fps，VFR 按真实 PTS 采样。每个出口另有无绝对路径的 manifest 和中文报告。绑定方式、时间规则与限额见 [ZV 稳定素材出口契约](docs/ZV_MEDIA_OUTLETS.md)。

### ZV H3 采访表与独立反推

`ZV H3 采访表` 只收集、对齐、整理用户输入，输出中文 `user_prompt` 和素材事实 `material_context_json`。空项省略，不补剧情、不生成系统词。三个独立反推阶段依次理解素材、整理中文用户意图、生成最终 H3 提示词；每阶段系统词独立可换。当前只保留 `ZVH3InterviewFormV2`，已移除旧基础采访表和旧结构化计划编译器，不再双版本并存。

时间和帧数来自素材台窗口；`reference_plan` 单独控制真实素材路由。模板预接首/尾帧、9 图、3 视频及其原声、驱动音频和3路独立参考音频，换素材后点击“检测并对齐素材”。详见[采访表与三阶段接线](docs/H3_INTERVIEW.md)及[分段长视频/蒙版指南](docs/LONG_VIDEO_GUIDE.md)。旧工作流文件不被自动改写，请使用当前模板。

### Animate 原流一次成片（测试版）

[Animate 一次成片附加方案](docs/ANIMATE_ONCE.md) 将素材台已剪好的视频与图片一一配对，按左右顺序执行原 WanAnimatePlus 工作流并合成视频。选择“硬切”或“原生 21 帧承接”；一个遮罩总开关控制整条视频，关闭跑动作迁移，开启后逐段填写目标词和参考源帧。原流的补帧、裁回、SAM/SeC 和采样链继续使用；不改 Plus，实际输出有帧数差异时明确报告。云端/GPU 生成效果仍待验证。

### 导演台流程辅助节点

- **ZF文本动态多路点选**：显示 1–32 路文本输入，按钮选择哪一路，执行时就只请求这一条路线；“空文本”按钮明确输出 `""` 和路线编号 `0`。
- **ZFI**：1–32 路独立的纯前端虚拟转接排；每一路都和官方转接点一样，排队执行时直接解析到上游，不产生后端节点、执行步骤或数据缓存。输出可正常多分线；工作流只保存数量、通道名称和接线拓扑。单填“图片”会自动编号为“图片1…图片N”，也可用 `|`、逗号或换行逐路命名，空值显示序号。
- **ZI 人像提示词生成器**：可复用于多种图像模型，不经过反推也能从可编辑素材目录直接随机并组装人像提示词。`数量` 为 1 时输出当前提示词；大于 1 时输出同等数量的提示词列表，第一条沿用当前选择，后续条目随机生成且保留所有锁定项，可直接驱动下游节点连续出图；数量另存于节点属性作为恢复保障，切换工作流后不会回到 1。添加界面支持首页项目固定、分段随机/锁定/停用和单项锁定；双击任意素材卡片即可原地编辑，“节点修复”可统一恢复内置原文。
