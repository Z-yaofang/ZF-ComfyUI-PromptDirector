...
旧工程里的 `ZVPictureSlotOutlet`、`ZVVideoSlotOutlet`、`ZVAudioSlotOutlet` 及其映射继续可执行；素材台仅在确实存在旧槽位时显示折叠的“旧版固定槽位（仅兼容）”面板，不再把槽位选择暴露为日常发送流程。详见 [旧版固定槽位兼容](docs/ZV_MEDIA_SLOTS.md)。
...
`ZV H3 基础采访表` 显式连接素材台的 `media_project`，把“想生成什么、必须保留/改变什么、素材各自承担什么用途”整理为 Stage①素材理解、Stage②导演审校、Stage③最终 H3 编译三组可接线文本。完整采访表常驻在大节点内，左右字段区各自滚动，点击画布不会关闭或丢失草稿；可从 8 个基础配方一键分配素材角色，也可让一个素材承担多个用途。素材编号与名称直接读取素材台工程，不复制、不重新导入或探测媒体。

时间和帧数不在采访表重复填写：它以素材台处理窗口为唯一权威，执行时验证 H3 单段的 24 fps、48–360 帧、最长 15 秒和帧网格。新模板把采访表的 `reference_plan` 接到单个 `ZV H3 素材对齐出口（官方容量）`；模板作者只需一次性预接官方完整的首/尾帧、9 图、3 视频、3 路同号视频原声、驱动音频和 3 路独立参考音频，玩家换素材后点“检测并对齐素材”即可，不再逐项创建出口或依赖 ZFI。`6 图 + 3 视频 + 3 音频` 只是常用选择档，不是硬上限。详见 [H3 基础采访表与接线](docs/ZV_H3_INTERVIEW_FORM.md)。原 `ZVH3FocusCompiler` 节点类型和严格计划契约保持不变，显示名改为 `ZV H3 结构化计划编译器（高级）` 以免混淆。
