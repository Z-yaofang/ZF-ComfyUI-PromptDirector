...
选择用途 **故事分镜图**，视觉方法推荐 **漫画页与动作转场**。它不是生成多张独立图片，而是生成**一张包含全部分镜的复合宫格图**。“漫画页与动作转场”只提供镜头节奏和动作承接，故事分镜模式会覆盖其中的大格、小格和跨格建议，强制使用等宽等高宫格。
...
- 整页直出的故事分镜依赖成图模型执行能力：部分模型即使收到正确的 6 格或 8 格提示，也会把多个分镜合并成较少的视觉面板。要求格数百分之百准确时，需要逐格分别生成，再用宫格拼接节点组合图像批次。
