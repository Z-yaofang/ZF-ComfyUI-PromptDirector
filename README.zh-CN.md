...
- **ZF 人像提示词与人物资产**：可复用于多种图像模型，不经过反推也能从可编辑素材目录直接随机并组装人像提示词；`world_asset` 始终输出节点内完整的常规人物素材库，可直接接入世界观多路点选，不受当前随机结果影响。添加界面支持首页项目固定、分段随机/锁定/停用和单项锁定；双击任意资产卡片即可原地编辑，确认后修改会固定在该选项并同时进入提示词与完整资产库，“节点修复”可统一恢复内置原文。需要查看结果时可自行连接“展示任何”节点。
