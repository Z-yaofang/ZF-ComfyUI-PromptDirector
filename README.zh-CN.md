...
仓库同时提供当前本地模型接法图 `local-workflow-connection-v2.png`：本地多模态/VLM 节点把结果接入 `ZF 参考图像分析器.analysis_result`，本地文本模型负责把导演台任务写成最终提示词，后面继续经过相同的提示词整理、总开关、去空行、字符串转列表和结果展示链。一次测试只选择 API 版或本地版，避免同时保留第二个写作节点或另一条反推分支。
...
API② 与提示词整理器之间另有 `ZF 最终文本列表缓存（跨队列复用）`，提示词整理器再把结果送到最终展示区。它保存的是整组多任务成品提示词，不会像单文本缓存那样只留下最后一条；先“更新缓存”，再“使用缓存”会跳过 API②，直接把上一组结果送入后续整理和展示。旧的提示词总开关、字符串处理和字符串转列表链仍保留给二次反推与兼容测试使用。

最终文本列表缓存是 API② 前的闸门，不会替代后面的整理链：**更新缓存**会运行 API②，并继续经过验证、去空行、字符串转列表和最终展示；**使用缓存**只跳过 API②，后处理链仍然完整执行；**清空缓存**会安全阻断该分支，不会把空列表送进切换节点。
